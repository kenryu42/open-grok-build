  ...
  const usages = state.accounts
    ...
    .map((account) =>
      account.quota.credits
        ? Math.max(0, Math.min(100, account.quota.credits.creditUsagePercent))
        : 0,
    );
...
const cardActions = (account) => {
  ...
  const rename = async () => {
    const label = await modal({
      ...
      message: "Shown here and in OpenCode's account list. Local to this machine.",
      value: account.label,
      confirm: 'Save label',
    });
...
const accountCard = (account, index, isNewPending, refreshing) => {
  ...
  if (account.plan) {
    metaRow.append(element('span', 'plan-pill', account.plan));
  }
  ...
  if (account.quota) {
    ...
    const weekly =
      credits?.periodType === 'USAGE_PERIOD_TYPE_WEEKLY' ? credits : account.quota.weekly;
    ...
    if (!lastState) {
      accountsRoot.replaceChildren(
        element(
          'p',
          'grid-message',
          'Dashboard connection lost. Run /grok-build-accounts to reopen it.',
        ),
      );
    }
    if (!wasOffline) {
