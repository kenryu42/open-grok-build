...
async function errorDetail(response: Response) {
  ...
  if (!text) return '';
  // The proxy returns plain text as often as JSON, so a parse failure is expected.
  try {
...
function httpError(status: number, detail: string) {
  ...
  if (status === 401 || status === 403) {
    return `Imagine rejected the token (HTTP ${status}). Run /connect and choose Grok Build, or set GROK_BUILD_OAUTH_TOKEN${suffix}`;
  }
...
// try/catch is unavoidable here: the retry budget has to survive both rejected
// fetches and non-ok responses while still clearing the per-attempt timeout.
...
export async function generateImage(options: {
  ...
  const response = await requestWithRetry(
    ...
    `${(options.baseUrl ?? process.env.GROK_BUILD_IMAGINE_BASE_URL ?? 'https://api.x.ai/v1').replace(/\/+$/, '')}/images/${options.imageUrl ? 'edits' : 'generations'}`,
    {
      method: 'POST',
      headers: {
      ...
      body: JSON.stringify({
        model: process.env.GROK_BUILD_IMAGINE_MODEL ?? 'grok-imagine-image-quality',
